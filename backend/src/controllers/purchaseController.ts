...
export const createPurchase = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // check first purchase (✅ camelCase)
    const firstPurchaseAlready = await Purchase.findOne({
      buyerId,
      firstPurchase: true,
    }).session(session);
    const isFirstPurchase = !firstPurchaseAlready;

    // ✅ create purchase with ordered:true
      ...
      [
        {
          ...
          amount,
          idempotencyKey,
          firstPurchase: isFirstPurchase,   // ✅ camelCase
          purchaseIp,
        },
      ],
          ...
          [
            {
              userId: referrer._id,
              delta: +2,
              reason: "REFERRER_CREDIT",
              refId: String(purchase._id),
            },
            {
              userId: buyer._id,
              delta: +2,
              reason: "REFERRED_CREDIT",
              refId: String(purchase._id),
            },
          ],
          { session, ordered: true }
